class EveryYearJob < ApplicationJob
  queue_as :default

  def perform
    PlannedTransaction.where(every: 'year')
                      .where('EXTRACT("day" FROM start_date) = ?', Date.today.day)
                      .where('EXTRACT("month" FROM start_date) = ?', Date.today.month)
                      .in_batches do |elements|
                        if elements.present?
